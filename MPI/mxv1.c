...
int main(int argc, char *argv[])
{
  ...
  if (me == 0) {
    ...
    for (proc = 1; proc < num_procs; proc++) {
      MPI_Send(&M[proc*sz], sz, MPI_DOUBLE, proc, 13, MPI_COMM_WORLD);    //Envias una parte filas de matriz M(de nº de proc a nº de proc + nº de filas a enviar)
      MPI_Send(&v[proc*mb], mb, MPI_DOUBLE, proc, 89, MPI_COMM_WORLD);    //Envias una parte de filas de vector v(de nº de proc a nº de proc + nº de filas a enviar)
      MPI_Send(x, n, MPI_DOUBLE, proc, 25, MPI_COMM_WORLD);     //Envia el elemento x
    }
  } else {
