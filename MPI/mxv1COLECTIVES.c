...
//Almacena matriz por filas y reparte por bloques de filas entre los procesos.
//CON COMUNICACIONES COLECTIVAS
//MUCHO MAS RAPIDO QUE MXV2C
...
int main(int argc, char *argv[])
{
  ...
  /*} else {
    MPI_Recv(Mloc, sz, MPI_DOUBLE, 0, 13, MPI_COMM_WORLD, MPI_STATUS_IGNORE);   //Recibos equivalentes...
  ...
  MPI_Scatter(M,sz,MPI_DOUBLE,Mloc,sz,MPI_DOUBLE,0,MPI_COMM_WORLD);//Envias con un reparto una parte filas de matriz M(de nº de proc a nº de proc + nº de filas a enviar)
  MPI_Scatter(v,mb,MPI_DOUBLE,vloc,mb,MPI_DOUBLE,0,MPI_COMM_WORLD);//Envias con un reparto una parte de filas de vector v(de nº de proc a nº de proc + nº de filas a envi
  MPI_Bcast(x,n,MPI_DOUBLE,0,MPI_COMM_WORLD); //Difusion de x
  ...
  for (iter = 1; iter <= num_iter; iter++) {
    ...
    MPI_Allgather(xloc,mb,MPI_DOUBLE,x,mb,MPI_DOUBLE,MPI_COMM_WORLD);   //Multirecogida de xloc para que este en los mismos sitios que x
...
#define ABS(a) ((a) >= 0 ? (a) : -(a))
  ...
  MPI_Reduce(&norma,&aux,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);    //Recoje valor de todos los procesos en uno, en este proceso se hace una operacion matematica (norma + aux se deja en p0)
