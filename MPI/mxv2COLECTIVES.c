...
//Matriz se almacena por columnas y se reparte por bloques de columnas 
...
//--------------------------------------------------------------------------------------------------------------------------------------
  ...
  MPI_Scatter(M,sz,MPI_DOUBLE,Mloc,sz,MPI_DOUBLE,0,MPI_COMM_WORLD);     //Envia un reparto de M por columnas 
  MPI_Scatter(x,nb,MPI_DOUBLE,xloc1,nb,MPI_DOUBLE,0,MPI_COMM_WORLD);    //Envia un reparto de x por filas 
  ...
  for (iter = 1; iter <= num_iter; iter++) {
    ...
    MPI_Reduce(xloc2,x,n,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);  //Recoje valor de todos los xloc y los suma en x
    if(me==0){
      for(i=0;i<n;i++){
        x[i]+=v[i];       //a x de process 0 le añades v
      } 
    } 
    MPI_Scatter(x,nb,MPI_DOUBLE,xloc1,nb,MPI_DOUBLE,0,MPI_COMM_WORLD);  //Reparto del vector a los demas processos para la siguiente iteracion
