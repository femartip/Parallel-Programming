...
//Implementacion paralela usando esquema maestro-trabajador bloqueante donde el maestro solo reparte, no hace trabajo.
                   ...
                   int me, int np)
{
  ...
  if (me == 0) {
    ...
    while ( num_row < h ) {
      ...
      MPI_Send(B, w, MPI_BYTE, 0, num_row, MPI_COMM_WORLD);           //Para que proceso maestro sepa que fila envia el trabajador, este la añade a su etiqueta.
      /* Receive the number of the next row to be computed */
      MPI_Recv(&num_row, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);    //El proceso trabajador no sabe que ya no tiene que procesar mas filas y se quedara esperando eternamente.
    }
