...
//Implementacion del mismo algoritmo(newton), pero en este caso queremos que el maestro tambien trabaje. Para ello tenemos que hacer que la implementacion maes.-trab. sea no bloqueante.
//VERSION MAS RAPIDA.
...
void escribe_pgm(char nom[], int ancho, int alto, Byte *imagen, int max, int col)
{
  ...
  else
  {
    ...
    {
      if (col < 0)
      { /* Se utiliza col como semilla para una paleta aleatoria */
        srand(col);
        for (i = 0; i < 256; i++)
          for (j = 0; j < 3; j++)
            pal[i][j] = rand() & 255;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#define A(i, j) A[(i)*w + (j)] //Para poder usar A como una matriz
                   ...
                   int me, int np)
{
  ...
  double complex z0;
  double ix, iy;
  int next_row, num_row, rows_done, proc, acabado;
  ...
  if (me == 0)
  {
    ...
    for (rows_done = 0; rows_done < h; rows_done++)
    {
      ...
      MPI_Request req;
      MPI_Irecv(B, w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &req);     //Iniciar recepcion no bloqueante
      MPI_Test(&req, &acabado, &status);
      while (acabado == 0 && next_row < h)    //Mientras no se ha recibido nada y quedan filas por procesar
      {
        for (j = 0; j < w; j++)
        {
