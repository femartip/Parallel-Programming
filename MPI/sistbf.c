...
//Version paralela donde matriz del problema se distribuye por bloques de filas 
...
int main(int argc, char *argv[])
{
  ...
  MPI_Bcast(b, n, MPI_DOUBLE, 0, MPI_COMM_WORLD);    //Difunde b a todos los procesos                

  MPI_Scatter(A[0], mb * n, MPI_DOUBLE, Aloc[0], mb * n, MPI_DOUBLE, 0, MPI_COMM_WORLD);    //Reparte matriz A a todos los porocesos en bloques de mb*n
  ...
  rc = lu(Aloc, n, iproc, p, mb);           //Funcion que llama a un broadcast
  CHECK(rc, 6);
  ...
  rc = triInf(Aloc, b, n, iproc, p, mb);      //Hace un brodcaste para que se sobreescriba b por Ly
  CHECK(rc, 8);
  ...
  rc = triSup(Aloc, b, n, iproc, p, mb);    //Hace brodcast para sobreescribir el vector y(b) con x, esta sera la solucion
  CHECK(rc, 9);

  if(iproc == 0){
    teje = MPI_Wtime() - teje;
    printf("sistbf tarda:%f\n", teje);
  } 
