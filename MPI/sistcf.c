...
//Version paralela donde matriz del problema se distribuye con una distribucion ciclica por filas. 
...
/*Dado un índice de fila i de la matriz global A, la función owner debe devolver el índice del proceso que
tiene esa fila en su matriz local Aloc.*/
...
/*Dado un índice de fila i de la matriz global A, la función localIndex debe devolver el índice de dicha
fila en la matriz local Aloc del proceso propietario de la fila.*/
...
/*modificar la función numLocalRows, que devuelve el número de filas locales de la
matriz en un proceso*/
...
int main(int argc, char *argv[])
{
  ...
  double teje = MPI_Wtime();      //Medir prestaciones
  ...
  for(int i=0,iloc=0;i<n;i+=p,iloc++){      //Distibucion de matriz ciclicamente por filas 
    MPI_Scatter(A[i],n,MPI_DOUBLE,Aloc[iloc],n,MPI_DOUBLE,0,MPI_COMM_WORLD);    //Este reparto cada vez que se ejecuta reparte ciclicamente entre p procesos las primeras o filas
  } 
